...
def compute_variance(path, center=False, flip=False):
    #sigma=0.0
    #pwArray=
    pwArray=np.zeros((3,227,227))
    print(pwArray.shape)
    print(pwArray)
	...
	image = np.asarray(Image.open(i)).transpose(2, 0, 1)
	print(image.shape)
	if center:
	...
	print(np.power(image,2).shape)
	print(pwArray.shape)
	pwArray +=np.power(image,2)

    pwArray=np.sqrt(pwArray/len(i))
    print(len(i))
    return pwArray
pwA=compute_variance(pathList)
...
def log_result():
    ...
    while True:
		...
		buftuple=(mean_error,mean_loss)
		logArray.append(train_count)
		logArray.append(buftuple)
		print(logArray)
            ...
            if val_count % 10 == 0:
                mean_loss = val_loss * args.val_batchsize / 10
                mean_error = 1 - val_accuracy * args.val_batchsize / 10
                print(file=sys.stderr)
                print(json.dumps({'type': 'val', 'iteration': train_count,
                                  'error': mean_error, 'loss': mean_loss}))
		buftuple=(mean_error,mean_loss)
		logArray2.append(train_count)
		logArray2.append(buftuple)
		print(logArray2)
                sys.stdout.flush()
...
pickle.dump(pwA, open('sigma.npy', 'wb'), -1)
